...
**Status**: Partially Implemented (core server shipped; VS Code extension client deferred)
...
The core language server shipped in `src/lsp/` and is exposed as the
`cclint-lsp` bin (`cclint-lsp --stdio`). What landed vs. what remains:

**Shipped (P0):**

- `textDocument/publishDiagnostics` — the existing rule pipeline
  (`createRules(config)` → `RulesEngine` over a `ContextFile` built from the
  document's text + path) runs on open/change/save; violations are mapped to LSP
  diagnostics via a pure, unit-tested `src/lsp/diagnostics.ts` (severity mapping
  and 1-based cclint → 0-based LSP position mapping).
- `textDocument/codeAction` — quick fixes for violations that carry a structured
  `Violation.fix`, producing the corresponding `WorkspaceEdit`.
- `workspace/configuration` (partial) — `.cclintrc.json` / presets are loaded
  via `ConfigLoader` relative to the edited document; live push-based
  `didChangeConfiguration` is not wired.

**Deferred:**

- `textDocument/hover` (P1), `textDocument/completion` (P2),
  `textDocument/formatting` (P2).
- The separate `cclint-vscode` extension client and syntax grammar. Any editor
  with a generic LSP client can launch `cclint-lsp --stdio` today.

**Implementation note / deviation:** the LSP adapter constructs the
`RulesEngine` **without** per-rule severity overrides (matching the MCP, watch,
and Action adapters). The engine applies a severity override by reconstructing
each violation, which discards its structured `fix`; keeping fixes intact is
required for code actions, so diagnostics reflect each rule's intrinsic
severity. If push-based per-rule severity in the editor is wanted later, the
engine's override path should be changed to preserve `fix`.
