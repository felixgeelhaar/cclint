# Backlog

Working list of features considered for cclint. Shipped items are kept here for
a short while as a record, then pruned; see [CHANGELOG.md](../CHANGELOG.md) for
the authoritative release history.

## Open

- Web playground (WASM + hosting) and remote pack registry / `pack publish`
  remain deferred.

## Shipped

- ✅ **Local community rule packs** (v0.23.0) — `cclint pack create|install|list`,
  `extends` resolution for `.cclint/packs/`.
- ✅ **Metrics + project presets + CI snippets** (v0.22.0).
- ✅ **AI breadth catch-up** (v0.21.0) — `lint --ai`, `lint --fix --ai`,
  config `ai`, suggest flags, Ollama provider, VS Code command palette wrappers.
- ✅ **Analyze drafts + VS Code marketplace readiness** (v0.20.0) —
  `analyze --draft` / `--write`, Scaffolder.preview, VSIX CI artifact.
- ✅ **Glob ignore + suggest/analyze + VS Code scaffold** (v0.19.0).
- ✅ **Parity + product rules** (v0.18.0) — Action/CLI parity, docs hygiene,
  `import-context-cost` + `enforcement-hint`.
- ✅ **Anthropic / AGENTS.md catch-up** (v0.17.0).
- ✅ **Project-wide lint / LSP / presets / SARIF** (v0.16.0).
- ✅ **Claude Code Extended Features** (v0.11.0).
- ✅ **Watch / init / hooks / interactive fix / explain / diff** (earlier).
