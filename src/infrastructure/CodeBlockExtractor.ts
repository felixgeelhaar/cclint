...
export class CodeBlockExtractor {
      ...
      if (inCodeBlock && this.isCodeBlockEnd(line, indentLevel)) {
        ...
        const codeBlock = new CodeBlock(
          ...
          context,
          {
            ...
            isAntiPattern: context.toLowerCase().includes('anti-pattern') || 
   ...
   */
  private parseCodeBlockStart(line: string): { language: string; indentLevel: number } | null {
  ...
  private getContext(lines: string[], codeBlockIndex: number): string {
    ...
    for (let i = codeBlockIndex - 1; i >= 0 && contextLines.length < maxContextLines; i--) {
  ...
  public extractCodeBlocksByLanguage(
    ...
    return allBlocks.filter(block => 
      this.normalizeLanguage(block.language) === normalizedLanguage
    );
  }
